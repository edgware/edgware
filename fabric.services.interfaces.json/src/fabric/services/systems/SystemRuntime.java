...
public class SystemRuntime extends Fabric implements ISubscriptionCallback, IClientNotificationHandler {
    ...
    public SystemRuntime(SystemDescriptor systemDescriptor, Object client, String className,
        ...
        boolean doQueryLocal = Boolean.parseBoolean(config("fabric.system.runtime.queryLocal", "false"));
    ...
    public void handleSubscriptionEvent(ISubscription subscription, int event, IServiceMessage message) {
        ...
        if (event == IServiceMessage.EVENT_SUBSCRIPTION_LOST) {
            ...
            try {
                ...
                logger.log(Level.FINE, "Failed to unsubscribe from feed [{0}] (input feed [{1}]): {2}", new Object[] {
                        outputFeed, e.getMessage()});
                logger.log(Level.FINEST, "Full exception: ", e);
            }
    ...
    /**
     * Unsubscribes from one or more output feed services from all input feed services.
     *
     * @param outputFeedServices
     *            the list of output feed services.
     *
     * @return the table of input feeds that have been unwired, and the corresponding output feed IDs.
     *
     * @throws Exception
     */
    public HashMap<ServiceDescriptor, List<ServiceDescriptor>> unsubscribe(ServiceDescriptor[] outputFeedServices)
        ...
        /* If output feed services have been specified... */
        if (outputFeedServices != null && outputFeedServices.length > 0) {
            ...
            for (ServiceDescriptor outputFeed : outputFeedServices) {
                ...
                ServiceDescriptor unwiredInputFeed = systemServices.unwireInputFeed(outputFeed);

                /* If the unsubscribe was successful */
                if (unwiredInputFeed != null) {

                    /* Record it */
                    @SuppressWarnings("unchecked")
                    List<ServiceDescriptor> unwiredFeeds = fabric.lookupSublist(unwiredInputFeed, allUnwired);
                    unwiredFeeds.add(outputFeed);
                }
            }
        }
    ...
    /**
     * Unsubscribes from one or more output feed services whose routes are broken by the loss of the specified node.
     *
     * @param node
     *            the lost node.
     *
     ...
     * @throws Exception
     */
    public HashMap<ServiceDescriptor, List<ServiceDescriptor>> unsubscribe(String node) throws Exception {

        Fabric fabric = new Fabric();
        HashMap<ServiceDescriptor, List<ServiceDescriptor>> allUnwired = new HashMap<ServiceDescriptor, List<ServiceDescriptor>>();

        ServiceDescriptor[] keys = systemServices.wiredInputFeeds().keySet().toArray(new ServiceDescriptor[0]);
        for (ServiceDescriptor serviceDesc : keys) {

            /* Get the route used by this subscription */
            ISubscription subscription = systemServices.wiredInputFeeds().get(serviceDesc);
            List route = Arrays.asList(subscription.route());

            /* If the route contains the lost node... */
            if (route.contains(node)) {

                /* Attempt to unsubscribe */
                ServiceDescriptor unwiredInputFeed = systemServices.unwireInputFeed(serviceDesc);
    ...
    public HashMap<ServiceDescriptor, ISubscription> wiredInputFeeds() {
        ...
        return systemServices.wiredInputFeeds();
    ...
    private boolean isValidTarget(ServiceDescriptor descriptor) {
        ...
        ServiceFactory sf = FabricRegistry.getServiceFactory(QueryScope.DISTRIBUTED);
    ...
    private String[] getRouteNodes(ServiceDescriptor serviceDescriptor) throws Exception {
        ...
        PlatformFactory platformFactory = FabricRegistry.getPlatformFactory(queryScope);
        ...
        RouteFactory routeFactory = FabricRegistry.getRouteFactory(queryScope);
