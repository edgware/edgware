...
public class AutoDiscoveryRequestFablet extends FabricBus implements IFabletPlugin, ICallback {
    ...
    private void openRequestSockets() {
        ...
        Node localNode = FabricRegistry.getNodeFactory(QueryScope.LOCAL).getNodeById(myNodeName);
     ...
     *            message to be sent
     * @param nodeIpMapping
     *            ipmapping information for opening multicast socket
     */
    ...
    public void run() {
        ...
        if (autoDiscoveryRequestEnabled) {
            ...
            while (isRunning) {
                ...
                for (Iterator<String> iter = oldFailedRequests.iterator(); iter.hasNext();) {
                ...
                for (Iterator<String> iter = failedRequests.iterator(); iter.hasNext();) {
                    String request = iter.next();
