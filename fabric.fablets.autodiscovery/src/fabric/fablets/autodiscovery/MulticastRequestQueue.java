...
public class MulticastRequestQueue {
    ...
    protected synchronized void addMsg(MulticastMessage autoDiscoveryMulticastMessage) {
        ...
        if (queue.size() != maxQueueDepth) {
            ...
            logger.log(Level.WARNING, "Overflow on discovery request queue - discarding message : {0}",
                    new Object[] {autoDiscoveryMulticastMessage.toString()});
        }
    ...
    protected synchronized MulticastMessage nextMessage() {
        ...
        logger.log(Level.FINEST, "Removed [{0}] from queue", response.toString());
        return response;
    }
