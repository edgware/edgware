...
java -Dconfig=%FABRIC_HOME%\bin\config\CLI.properties -Dfabric.config=%FABRIC_HOME%\osgi\configuration\fabricConfig_default.properties -Djava.util.logging.config.file=%FABRIC_HOME%\osgi\configuration\logging.properties -cp %CLASSPATH% fabric.tools.json.JsonCLI %1
