...
/**
 * Set of utility methods covering IP address related operations - anything from getting a list of local IP addresses to
 * comparing addresses to see if they are in the same network.
 */
public class NetworkUtils {
	 ...
	 *             If the localhost name cannot be resolved.
	...
	/**
	 * Return all valid interfaces for this local host. Note: List can include interfaces for both IPv4 and IPv6
	 * addresses.
	 * 
	 * @return a list of the valid interface names for this host or null in the event of an error.
	 * @throws UnknownHostException
	 *             If the localhost name cannot be resolved.
	 * @throws SocketException
	 */
	 ...
	 *             If the localhost name cannot be resolved.
	 * @throws SocketException
	 *             if an error occurs getting the list of local interfaces
	 */
	 ...
	 * @throws SocketException
	 *             if an error occurs getting the interface list.
	 */
	 ...
	 *            the prefix indicating the subnet mask (typically 8, 16, or 24 for IPv4).
	...
	public static byte[] buildSubNetMaskFromNetworkPrefix(short networkPrefixLength) {
		...
		int value = mask;
		byte[] maskBytes = new byte[] {(byte) (value >>> 24), (byte) (value >> 16 & 0xff), (byte) (value >> 8 & 0xff),
				(byte) (value & 0xff)};
	...
	/**
	 * Get the network prefix length (indicating the subnet mask) for the specified IP address.
	 * 
	 * @param ipAddress
	 *            the ip address for which the prefix should be identified.
	 * @return the network prefix length or 0 if not found.
	 * @throws UnknownHostException
	 *             if the ip address specified cannot be resolved to the local host.
	 * @throws SocketException
	 *             if an error occurs while attempting to identify the network prefix length.
	 */
	...
	/**
	 * Compute the network address given the specified IP address and subnet mask. Useful for comparing two IP addresses
	 * to determine if they are within the same subnet.
	 * 
	 * @param ipAddress
	 * @param subnetMask
	 ...
	 * @throws UnknownHostException
	 *             if the computed network address cannot be resolved to a valid InetAddress
	 */
	public static InetAddress computeNetworkAddress(InetAddress ipAddress, byte[] subnetMask)
			throws UnknownHostException {
