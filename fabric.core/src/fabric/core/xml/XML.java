...
/** Embedded XML "<" character */
	 ...
	 */
	protected boolean includePIs = true;
	...
	public String get(String path) {
		...
		if (path.indexOf('@') < 0 && !Pattern.matches("/.*/\\$(\\[.*\\]|$)", path)) {
			path = path + "/$";
		}
	...
	public String[] getPaths() {
		...
		if (documentElement != null) {
			...
			/* To hold the paths */
			ArrayList<StringBuffer> orderedPathsList = new ArrayList<StringBuffer>();

			/* To hold the first path */
			orderedPathsList.add(new StringBuffer());
			...
			Iterator<StringBuffer> i = orderedPathsList.iterator();
	 ...
	 */
	protected XMLNode toXML(XMLNode node, StringBuffer xml, String indent) {
	 ...
	 *            the list of paths.
	 */
	protected void toPaths(XMLNode node, int count, ArrayList<StringBuffer> paths) {

		/* Get the current path being built */
		StringBuffer currentPath = paths.get(paths.size() - 1);
		...
		if (node instanceof XMLElement) {
			...
			if (element.getSubnodes().size() > 0) {
				...
				for (int s = 0; s < element.getSubnodes().size(); s++) {
					...
					/* Start a new path */
					StringBuffer newPath = new StringBuffer(currentPath.toString());
					paths.add(newPath);
	...
	protected static String decodeEntityRefs(String xmlContent) {
		...
		StringBuffer entityRefBuffer = null;
		...
		for (int c = 0; c < xmlContentChars.length; c++) {
			...
			if (entityRefBuffer == null) {
				...
				if (xmlContentChars[c] == '&') {
					entityRefBuffer = new StringBuffer();
	...
	/**
	 * Locatate a node based upon its path.
	 * 
	 * @param path
