...
public class MqttEndPoint extends EndPoint implements MqttCallback {
    ...
    public void close() throws IOException, UnsupportedOperationException {
        ...
        try {
            ...
            logger.log(Level.FINER, "Cannot publish disconnect message: {0}", e.getMessage());
        ...
        try {
            ...
            logger.log(Level.FINER, "Cannot disconnect from the broker: {0}", e.getMessage());
            logger.log(Level.FINEST, "Exception: ", e);
        }
    ...
    public void connectionLost(Throwable t) {
        ...
        logger.log(Level.INFO, "MQTT connection to [{0}] (client [{1}]) lost; retrying {2} time(s)", new Object[] {
                config.getbrokerIpAddress(), config.getClient(), config.getConnectRetries()});
        ...
        logger.log(Level.FINE, "Retrying lost broker connection");
    ...
    private void mqttConnect() throws IOException {
        ...
        while (!connected) {
            ...
            try {
                ...
                logger.log(Level.FINER, "Connected to [{0}] as [{1}]", new Object[] {config.getbrokerIpAddress(),
                        config.getClient()});
                ...
                if (maxRetries != -1) {
                    ...
                    if (++retries > maxRetries) {
                        ...
                        logger.log(Level.INFO, "No connection to broker [{0}] as [{1}]", new Object[] {
                                config.getbrokerIpAddress(), config.getClient()});
