...
/**
 * Controls access to the persistence service in use - either local (using a JDBC connection)
 * or remote (using a Fabric connection).
 * 
 */
public class PersistenceManager {
	...
	/**
	 * Connects to a local Fabric Registry database node using JDBC. Only one connection
	 * is established per Fabric JVM.
	 * 
	 * @param dbUrl
		...
		synchronized(monitor) {
			if (! connectedToRegistry) 
			{
				try {
					...
					{  
						switch (config.getProperty(ConfigProperties.REGISTRY_TYPE)) {
						...
						case RegistryDescriptor.TYPE_SINGLETON:
							...
							break;
						default:
							logger.log(Level.SEVERE, "FAILED TO establish a REGISTRY of TYPE " + config.getProperty(ConfigProperties.REGISTRY_TYPE));
							break;
						}					
					}
