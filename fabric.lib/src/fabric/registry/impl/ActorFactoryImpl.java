...
public class ActorFactoryImpl extends AbstractFactory implements ActorFactory {
	...
	static {
		...
		remoteQueryInstance = new ActorFactoryImpl(false);
		/* Create an instance for local (singleton) Registry operations */
		localQueryInstance = new ActorFactoryImpl(true);
	}

	private ActorFactoryImpl(boolean queryLocal) {
	...
	public static ActorFactoryImpl getInstance(boolean queryLocal) {
		if (queryLocal) {
