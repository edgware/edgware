...
public abstract class AbstractFactory extends Fabric {
	...
	// Indicates if the queries are to be local only
			...
			try {
				...
				if (obj.getShadow() == null) {
					try {
						...
						logger.log(Level.FINE, "Object already exists and will be updated instead: {0}", obj);
	...
	public RegistryObject[] queryRegistryObjects(String sqlString, AbstractFactory factory) throws PersistenceException {
		...
		if (sqlString != null && sqlString.length() > 0) {
			results = PersistenceManager.getPersistence().queryRegistryObjects(sqlString, factory, localOnly);
		}
		return results;
	}
