...
public class FabricBus extends Fabric {
	...
	public SharedEndPoint connectHomeNode() {
		...
		int homeNodePort = Integer.parseInt(config(ConfigProperties.MQTT_LOCAL_PORT,
				ConfigProperties.MQTT_LOCAL_PORT_DEFAULT));
		NodeDescriptor nodeDescriptor = new NodeDescriptor(node, homeNodePort);
	...
	public void disconnectFabric() {
		...
		try {
			...
			logger.log(Level.WARNING, "Failed to close connection to Fabric Registry:\n", e1);
		}
		...
		try {
			...
			logger.log(Level.WARNING, "Failed to close connection to Fabric broker:\n", e);
		}
	...
	public SharedEndPoint connectNode(NodeDescriptor nodeDescriptor) {
		...
		if (nodeDescriptor == null) {
			...
			logger.log(Level.FINER,
					"Connecting to broker for node \"{0}\" on interface \"{1}\" at address \"{2}:{3}\"", new Object[] {
							nodeDescriptor.name(), nodeDescriptor.networkInterface(), nodeDescriptor.address(),
							"" + nodeDescriptor.port()});
			...
			try {
				...
				if (sharedEndPoint == null) {
					...
					logger.log(Level.FINE, "Connected to broker for node \"{0}\" interface \"{1}\" address: {2}",
