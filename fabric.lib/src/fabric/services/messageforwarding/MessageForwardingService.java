			...
			if (nextMessage != null) {
				...
				case FORWARD:
					...
					try {
						...
						busServices.sendFeedMessage(nextMessage.node(), nextMessage.descriptor(),
								nextMessage.message(), nextMessage.messageQos());

					} catch (Exception e) {

						logger.log(Level.WARNING, "Failed to send message to service \"{0}\" on node \"{1}\": {2}",
								new Object[] {nextMessage.descriptor(), nextMessage.node(), LogUtil.stackTrace(e)});
				...
				default:
					...
					logger.log(Level.WARNING, "Internal error, unsupported message action: {0}", nextMessage.action()
							.toString());
					break;
