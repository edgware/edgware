...
public class DynamicRoutingFactory implements IRoutingFactory {
	...
	public String[] getRouteNodes(String startNode, String endNode) {
		/* 
		 * This is a simple implementation of Dijkstra's Algorithm to
		 * determine the shortest available path between the specified
		 * nodes.
		 * 
		 * It current assumes a constant fixed weight for each edge in the
		 * network. It could be adapted to take into account attributes
		 * of the network, such as bandwidth/reliability.
		 * 
		 * All routes get the same Ordinal value (1).
		 */
		...
		NodeNeighbour[] nn  = FabricRegistry.getNodeNeighbourFactory(false).getAllNeighbours();
