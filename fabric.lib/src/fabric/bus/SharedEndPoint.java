...
public class SharedEndPoint extends EndPoint {
	...
	public void closeChannel(SharedChannel sharedChannel, boolean doForceClose) throws IOException,
		...
		if (sharedChannel != null) {
			...
			logger.log(Level.FINE, "Removing reference to channel {0}", channelKey);
			...
			if (doForceClose || sharedChannel.refCount() == 0) {
				...
				/* Close and remove it */
				logger.log(Level.FINE, "Closing channel {0} ({1} references)", new Object[] {channelKey,
			...
			logger.log(Level.FINEST, "Attempt to close null channel");
