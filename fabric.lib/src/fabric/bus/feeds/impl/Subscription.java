...
public class Subscription implements ISubscription, ICallback, IClientNotificationHandler {
	...
	public TaskServiceDescriptor subscribe(TaskServiceDescriptor serviceDescriptor, String[] route,
		...
		if (route == null) {
			...
			logger.log(Level.FINER, "Sending subscribe command: {0}", subscriptionMessage.toString());
	...
	public void handleMessage(Message message) {
		...
		logger.log(Level.FINEST, "Handling message from topic \"{0}\":\n{1}", new Object[] {message.topic,
			...
			if (parsedMessage instanceof IFeedMessage) {
				try {
					...
					logger.log(
							...
							new Object[] {callback.toString(), message, LogUtil.stackTrace(ce)});
				}
